...
public class Main {
    public static void main(String[] args) {
        ...
        drinkList.add(new MenuItem("1. Coke   | W", 2.5, "| 콜라"));
        drinkList.add(new MenuItem("2. Sprite   | W", 2.5, "| 사이다"));
        drinkList.add(new MenuItem("3. Milkshake   | W", 4.0, "| 밀크쉐이크"));


        List<MenuItem> dessertList = new ArrayList<>();

        dessertList.add(new MenuItem("1. InjeolmiTime   | W", 5.3, "| 솔티드 허니와 인절미가 포함된 아이스크림"));
        dessertList.add(new MenuItem("2. HakunaBanana   | W", 5.3, "| 솔티드 캐러멜과 바나나가 포함된 아이스크림"));
        dessertList.add(new MenuItem("3. ShackAttack   | W", 5.3, "| 진한 초콜릿과 초코릿 토핑이 포함된 아이스크림"));

        List<Menu> menuCategory = new ArrayList<>();
        //카테고리에 입력 된 값에 맞는 메뉴의 리스트를 담아줍니다.
        menuCategory.add(new Menu("1. Burger", burgerList));
        menuCategory.add(new Menu("2. Drinks", drinkList));
        menuCategory.add(new Menu("3. Desserts", dessertList));
